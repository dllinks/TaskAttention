...
class Trainer():
    ...
    def attention_batch_update(self,task_losses,task_gradients,l_ratio,acc_ratio,task_accuracies): 
        ...
        # find index of attention_max attention vector and its corresponding loss
        # max_index               = (attention_vector == attention_max).nonzero(as_tuple=True)[0]
        # print ('max_index',max_index)
        # print ('corresponding loss',task_losses[max_index])
        # print ('corresponding accuracy',task_accuracies[max_index])
        
         # find index of max loss task and its rank in attention vector
        # max_loss                     = np.max(task_losses.cpu().detach().numpy())
        # max_accuracy                 = np.max(task_accuracies.cpu().detach().numpy())
        # max_loss_index               = (task_losses == max_loss).nonzero(as_tuple=True)[0]
        # max_accuracy_index           = (task_accuracies == max_accuracy).nonzero(as_tuple=True)[0]
        # print ('corresponding attention ',attention_vector[max_loss_index])
        # print ('corresponding attention ',attention_vector[max_accuracy_index])
        # attention_vector_            = np.sort(attention_vector.cpu().detach().numpy())
        # max_loss_rank                = np.where(attention_vector.cpu().detach().numpy()[max_loss_index]==attention_vector_)
        # max_accuracy_rank            = np.where(attention_vector[max_accuracy_index]==attention_vector_)
        # print ('max_loss_rank ',max_loss_rank)
        # print ('max_accuracy_rank ',max_accuracy_rank)
        ...
        # self.history["task-losses"]["train"].append(task_losses.cpu().detach().numpy())
        # self.history["max-loss-rank"]["train"].append(max_loss_rank)

        
        # self.history["task-gradients"]["train"].append(task_gradients)
        
   
        # task_gradients=flatten(task_gradients)
        
        # task_gradients  = flat_params(task_gradients)
        # print('task_gradients shape',task_gradients[0].shape,task_gradients[1].shape,task_gradients[2].shape,task_gradients[3].shape)
        # cosine_sim      = cosine_similarity(task_gradients)
        # cosine_distance = 1-cosine_sim

        # self.history["batch-csd"]["train"].append(cosine_distance)
        
        
        #self.batch_entropy = entropy(attention_vector.cpu().detach().numpy(), base=2)
